...
retry:
	...
	if (spa->spa_root_vdev->vdev_child[0]->vdev_ops ==
		...
		/*
		 * XXX we will also want to include the label config in the
		 * per-txg metadata, which can be generated by calling
		 * spa_config_generate(spa, vd, txg, B_FALSE);
		 */
		object_store_end_txg(spa, txg);
		return (0);
	}
#endif
