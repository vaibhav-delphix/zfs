...
fn main() {
    ...
    error!(
        "logging level ERROR enabled; local timezone is {}",
    ...
    // be recovered from.
    warn!(
        "logging level WARN enabled; local timezone is {}",
        chrono::Local::now().format("%Z (%:z)")
    );
    ...
    info!(
        "logging level INFO enabled; local timezone is {}",
        chrono::Local::now().format("%Z (%:z)")
    );
    ...
    // call to S3.
    debug!(
        "logging level DEBUG enabled; local timezone is {}",
        chrono::Local::now().format("%Z (%:z)")
    );

    // trace!() can be used indiscriminately.
    trace!(
        "logging level TRACE enabled; local timezone is {}",
        chrono::Local::now().format("%Z (%:z)")
    );
